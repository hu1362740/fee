...
const getPVCount = RouterConfigBuilder.routerConfigBuilder('/api/pv/count', RouterConfigBuilder.METHOD_TYPE_GET, async (req, res) => {
  ...
  st = st / 1000
  et = et / 1000
  const pvTotal = await MPageView.getPVInRange(projectId, st, et)
  res.send(API_RES.showResult(pvTotal))
})

const getPVTrend = RouterConfigBuilder.routerConfigBuilder('/api/pv/trend', RouterConfigBuilder.METHOD_TYPE_GET, async (req, res) => {
  const projectId = _.get(req, ['fee', 'project', 'projectId'], 1)
  ...
  let rawRecordList = await MPageView.getRawRecordListInRange(projectId, st, et, countType)
